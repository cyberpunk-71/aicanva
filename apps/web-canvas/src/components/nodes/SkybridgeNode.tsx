...
export function SkybridgeNode({ id, data }: NodeProps & { data: Record<string, unknown> }) {
  ...
  // Generate iframe src whenever data changes
  useEffect(() => {
      ...
      <div className="relative px-4 py-3 border-b border-white/5">
        ...
        <div className="relative flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => {
              // Force reload by creating new blob URL
              const widgetHtml = nodeData.widgetHtml as string;
              if (widgetHtml) {
                const blob = new Blob([widgetHtml], { type: "text/html" });
                setIframeSrc(URL.createObjectURL(blob));
              }
            }} className="p-1.5 rounded-lg text-slate-600 hover:text-violet-400 hover:bg-violet-500/10 transition-all"><RefreshCw size={12} /></button>
